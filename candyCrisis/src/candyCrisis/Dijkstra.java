...
public class Dijkstra {
	...
	/*
	 * Constructor: Dijkstra Paramaters: None Description: A default constructor
	 * that does nothing
	 */
	public Dijkstra() {
	...
	/*
	 * Function: runDikstra Parameters:
	 * 
	 * @String startNode: A 15 character single line representation of the board
	 * state Returns:
	 * 
	 * @ List<String>: A list of 15 character board states that is the path from
	 * the start to the goal. Description: A function that runs the Dijkstra
	 * algorithm until all paths lead to no goal or the shortest path to a goal
	 * node is reached.
	 */
	public List<String> runDijkstra(String startNode) {
		iterations = 0; // TODO remove iteration as this is for loops testing
	...
	/*
	 * Function: containsName Params:
	 * 
	 * @ final List<Node> list: list to check for containing name
	 * 
	 * @ final String name: name to check if in list Returns:
	 * 
	 * @ boolean: state of the list containing the name in the node name field
	 * Description: A function that filters the node list for elements that
	 * contain the passed name. Filters for the first node and checks if there
	 * was any to confirm existence
	 */
	public Node containsName(final List<Node> list, final String name) {
		if(list.isEmpty())
		{
			return null;
		}
		return  (Node) list.stream().filter(o -> o.name.equals(name)).findFirst().orElse(null);
	}

	/*
	 * Function getElementWithName Params:
	 * 
	 * @ final List<Node> list: list to get element from
	 * 
	 * @ final String name: name of the node to get from the list Returns:
	 * 
	 * @ Optional<Node>: returns the node if it is possible to get. Run only
	 * after using containsName to confirm existence. Description: A function
	 * that gets the first node with the name tag on the passed list. Optional
	 * since the method to fetch can find nothing. Should only be run after
	 * running containsName to confirm existence of such an element
	 */
	 ...
	 * 
	 * @ String heuristic: estimated cost to get to the new node Description: A
	 * function that inserts nodes into a sorted list in the first position from
	 ...
	 * 
	 * @ Node: node to add into the sorted list Description: A function that
	 * inserts an existing node into a sorted list at the first position from
	 * the left that the node is greater than. This allows for the list to stay
	 * in a sorted state
	 * 
	 */
	...
	public void listAdd(List<Node> list, Node current) {
		list.add(current);
	}

	/*
	 * Function: debugger Description: A function that prints out the contents
	 * of the closed and open list. Useful for the testing of the Dijkstra
	 * looping is functioning properly
	 */
	 ...
	 * 
	 * @ List<Node> list: list to print Description: A function that loops over
	 * a node list and outputs a human readable list to the console instead.
	 */
	 ...
	 * Returns:
	 * 
	 * @ List<String>: A list of every state from the end state to the start
